...
export const games: GameEntry[] = [
  ...
  },
  {
    ...
    copy: {
      ko: {
        title: '금고 설계자',
        tagline: '당신이 설계한 금고를, 당신이 먼저 털어야 세상에 공개됩니다. 짓는 사람과 터는 사람이 같은 사람인 디펜스 게임입니다.',
        about: [
          '보통의 디펜스 게임은 <b>앉아서 봅니다</b>. 타워를 놓고 나면 할 일이 없습니다. 이 게임은 반대편을 줍니다 — 금고를 짓고 나면 <b>직접 도둑이 되어 그 금고를 털어야</b> 합니다. 못 털면 그 금고는 세상에 나가지 못합니다.',
          '규칙 한 줄이 네 가지를 동시에 막습니다. 맵이 고갈되지 않고(사람이 만듭니다), 보기만 하는 구간이 없고, 짓는 감각과 뚫는 감각이 같이 자라고, <b>아무도 못 뚫는 금고가 원리적으로 존재할 수 없습니다</b>. 벽으로만 틀어막은 금고는 자기도 못 뚫으니까요.',
          '보안장치는 다섯 종인데 <b>하나만으로는 아무것도 못 합니다</b>. 카메라는 발각만 걸고 때리지 않습니다. 레이저는 발각된 것만 쏩니다. 그래서 둘이 만나야 비로소 방어가 됩니다. 경비견은 느린 것만 무니까 잠금문으로 세워야 물립니다. 설명서를 읽을 필요가 없습니다 — 관계가 화면에서 그대로 보입니다.',
          '도둑은 여섯인데 <b>역할이 겹치지 않습니다</b>. 그림자는 카메라에 안 잡히고, 자물쇠공은 문 앞에서 멈추지 않고, 곡예사는 압력판을 밟지 않습니다. 대신 유리 바닥은 <b>빠른 도둑만</b> 깨뜨립니다 — 처음으로 느린 것이 유리해지는 자리입니다.',
          '캠페인 <b>36개</b>가 여섯 장으로 나뉘어 있고, 한 금고가 한 가지를 가르칩니다. 앞의 몇 개는 일부러 <b>아무도 안 죽습니다</b> — \'카메라만으로는 막을 수 없다\'를 보여 주려면 실제로 막히지 않아야 하니까요.',
          '화면 전체가 <b>설계 도면</b>입니다. 스프라이트도 3D 모델도 없이 전부 선과 도형으로 그렸습니다. 금고 설계도니까 도면인 것이 스타일이 아니라 <b>설정</b>입니다.',
          '난이도는 눈으로 정하지 않았습니다. 금고마다 아홉 가지 편성을 자동으로 돌려 <b>몇 개가 통하는지</b> 재고, 전부 통하거나 아무도 못 뚫으면 다시 맞췄습니다. 36개 중 <b>못 깨는 금고는 하나도 없습니다</b>.',
          '내가 지은 금고는 <b>코드 한 줄(104자)</b>로 건넬 수 있습니다. 서버도 계정도 필요 없습니다. 단 <b>증명하기 전에는 내보내기가 잠깁니다</b>.',
          '2026년 9월, AI가 전부 만들었습니다 — 규칙, 금고 생성기, 밸런스 측정 도구, 캠페인 36개까지.',
        ],
        how: [
          '왼쪽에서 도둑을 눌러 <b>뽑고</b>, 한 번 더 누르면 <b>투입</b>합니다 — 예산 안에서',
          '한 명이라도 금고실(◆)에 닿으면 성공입니다. 한꺼번에 넣으면 한 방에 쓸립니다',
          '<b>일시정지 중에도 투입·개입이 됩니다</b> — 손이 느려서 지는 것은 이 게임이 묻는 실력이 아닙니다',
          '개입은 판당 <b>한 번</b>뿐입니다: 정전(전기만 3.5초) · 폭파(벽 한 칸) · 양동(개를 부름)',
          '🔨 설계에서 <b>끌어서</b> 벽을 칠합니다. 같은 것을 또 칠하면 지워집니다. 벽에도 값이 듭니다',
          '지었으면 <b>내 금고를 직접 털어야</b> 내보내기가 열립니다. 한 칸이라도 고치면 증명은 무효입니다',
        ],
      },
      en: {
        title: 'Vault Maker',
        tagline: 'The vault you designed goes public only after you break into it yourself. A defense game where the builder and the thief are the same person.',
        about: [
          'Most tower defense games leave you <b>watching</b>. Here you get the other half: once you build a vault, you become the thief and <b>have to break into your own design</b>. Fail, and it never goes public.',
          'That one rule solves four problems at once. Maps never run out (people make them), there is no passive stretch, building and breaking sharpen each other, and <b>an unbeatable vault cannot exist</b> — a wall-spam vault defeats its own author.',
          'Five security devices, and <b>none of them does anything alone</b>. Cameras only mark; they never hurt. Lasers only fire at what has been marked. Guard dogs bite only slow targets, so you need a locked door to make someone slow. No manual needed — the relationships are visible on screen.',
          'Six thieves with <b>no overlapping roles</b>: the ghost is invisible to cameras, the locksmith never stops at doors, the acrobat never triggers pressure plates. Glass floors break under <b>fast</b> thieves only — the one place where being slow is an advantage.',
          'A <b>36-vault campaign</b> across six chapters, each vault teaching one thing. The first few intentionally <b>kill nobody</b> — to show that cameras alone cannot stop you, nothing must actually stop you.',
          'The whole screen is a <b>blueprint</b>: no sprites, no models, everything drawn with lines and shapes. For a vault schematic that is not a style choice, it is the setting.',
          'Difficulty was not eyeballed. Every vault is auto-played with nine team compositions to measure how many get through; if all pass or none do, it gets retuned. <b>None of the 36 is unbeatable.</b>',
          'Your own vault travels as a <b>single 104-character code</b> — no server, no account. Export stays locked until you have proven it.',
          'Built entirely with AI in September 2026 — rules, vault generator, balance tooling and all 36 campaign vaults.',
        ],
        how: [
          'Click a thief on the left to <b>hire</b>, click again to <b>send them in</b> — within budget',
          'One thief reaching the vault (◆) wins. Send everyone at once and they get wiped at once',
          'You can hire and intervene <b>while paused</b> — reaction speed is not the skill being tested',
          'One intervention per run: blackout (electric only, 3.5s), breach (one wall), decoy (lure the dog)',
          'In 🔨 design mode, <b>drag</b> to paint walls. Paint the same thing again to erase. Walls cost budget too',
          'Once built, you must <b>break into it yourself</b> to unlock export. Change one tile and the proof is void',
        ],
      },
    },
  },
  ...
  },
  {
    ...
    copy: {
      ko: {
        title: '조스 오브 파이터즈',
        tagline: '옛 오락실 대전격투 — 직업이 곧 기술인 열 명의 싸움꾼, 일곱 곳의 무대, 시간이 멈추는 초필살기 컷인, 그리고 아홉 판을 지나가는 이야기.',
        about: [
          '1:1 대전격투 게임입니다. 내가 고른 한 명 말고는 전부 <b>CPU</b>가 잡습니다 — 아케이드로 아홉 명을 차례로 상대하거나, 상대와 무대를 골라 한 판만 붙을 수 있습니다. 난이도는 네 단계고 기본은 <b>보통</b>입니다.',
          '아케이드는 그냥 아홉 판이 아니라 <b>이야기</b>입니다. 조스클럽 회장이 "이긴 사람의 소원을 사 주겠다"며 대회를 열었고, 열 명에게는 저마다 나온 이유가 있습니다. 붙기 전마다 <b>그 상대에게만 하는 대사</b>가 오가고(짝마다 다릅니다 — 모두 90쌍), 마지막 상대는 늘 대회를 연 사람이며, 아홉을 다 이기면 <b>그 사람의 결말</b>이 나옵니다.',
          ...
          '커맨드는 옛 오락실 그대로입니다. <b>↓↘→ + 손</b>이 장풍, <b>→↓↘ + 손</b>이 대공기, <b>↓↙← + 발</b>이 파고드는 기술입니다. 커맨드가 손에 안 붙는 사람을 위해 <b>필살기 버튼(L)</b>을 따로 뒀습니다 — 방향을 같이 누르면 네 가지 필살기가 갈리고, 게이지가 가득일 때 방향 없이 누르면 초필살기가 나갑니다. 둘 중 아무 쪽으로나 쓰면 됩니다.',
          '초필살기를 쓰면 <b>시간이 멈춥니다.</b> 화면이 어두워지고 그 사람의 일러스트가 옆에서 밀고 들어오고, 기술 이름이 뜬 다음, 풀린 뒤에는 <b>느리게 흐릅니다</b>(KO 도 느리게 끝납니다). 컷인 그림 열 장은 이 게임을 위해 따로 그렸습니다.',
          '필살기는 한 사람당 <b>넷 + 초필살기 하나</b>이고, 전부 그 직업에서 나옵니다 — 민유는 <b>롤백</b>으로 무적으로 굴러 빠져나가고, 종범은 소포를 <b>포물선</b>으로 던지고, 노덕은 위에서 <b>내리꽂아</b> 앉아 막는 상대를 열고, 준원의 자석 집게는 <b>밀지 않고 끌어당기고</b>, 조스의 배당금은 맞히면 <b>게이지가 크게 차고</b>, 유정의 문제집은 <b>상대 머리 위에서</b> 떨어집니다. 기술마다 이펙트와 효과음이 다 다릅니다.',
          '막기는 버튼이 아니라 <b>뒤로 미는 것</b>이고, 하단은 앉아서, 점프 공격과 머리 위에서 오는 것은 서서 막아야 합니다.',
        ...
        how: [
          ...
          '필살기 버튼 L — 방향 없이/↓/뒤/앞 에 따라 네 가지가 갈립니다',
          '게이지가 가득일 때 방향 없이 L — 초필살기(컷인 + 슬로모션)',
          '옛날 방식도 그대로 — 장풍 ↓↘→ + 손 · 대공 →↓↘ + 손 · 초필 ↓↘→↓↘→ + 손',
  ...
  {
    // ⚠️폴더는 `englishzoo`, 슬러그는 `english-zoo` 로 **다르게** 둔다.
    //   같게 두면 사이트가 만드는 소개 페이지(/games/<슬러그>/)가 게임 폴더를 덮어쓴다(문주 게임 때 겪음).
    slug: 'english-zoo',
    released: '2026-09',
    tech: 'HTML · Web Audio · 실제 영어 발음(macOS TTS) · 이모지 그래픽',
    playPath: '/games/englishzoo/index.html',
    cover: '/games/englishzoo/cover.jpg',
    koOnly: true,
    hero: 'yujin',
    copy: {
      ko: {
        title: '영어 동물원',
        tagline: '영어를 듣고 맞는 그림을 찾는 낱말 게임 — 낱말 85개, 진짜 영어 발음, 그리고 틀려도 줄지 않는 목숨.',
        about: [
          '초등학교 영어는 3학년부터 시작합니다. 그래서 1학년에게 필요한 것은 단어 시험이 아니라 <b>영어 소리가 낯설지 않은 상태</b>입니다. 이 게임은 철자를 묻지 않습니다. 소리를 들려주고 그림을 고르게 합니다. 글자는 답을 맞힌 <b>뒤에</b> 보여 줍니다 — 소리로 고른 것을 글자로 확인시키는 순서입니다.',
          '발음은 브라우저가 읽어 주는 것이 아니라 <b>미리 만들어 둔 음성 파일</b>입니다. 브라우저 내장 음성은 기기마다 영어 목소리가 없거나 늦게 떠서, 아이 화면에서는 그대로 “눌렀는데 아무 소리도 안 남”이 됩니다. 낱말마다 보통 속도와 <b>천천히</b> 두 가지를 준비했고, 천천히는 음높이를 유지한 채 늘려 발음 본보기로 쓸 수 있게 했습니다.',
          '문제 순서에는 근거가 있습니다. 실제로 한 1학년 아이를 살펴보니 <b>elephant·teacher·student·shark 같은 긴 낱말은 알면서 tiger 와 lion 은 헷갈렸습니다.</b> 노래와 수업에서 자주 들은 쪽이 남은 것입니다 — 길이가 아니라 들은 횟수가 갈랐습니다. 그래서 이 게임은 아이가 이미 아는 낱말부터 내보내 첫 문제를 맞히게 하고, 그다음에 새 낱말로 갑니다.',
          '그리고 <b>헷갈리는 짝은 일부러 같이 냅니다.</b> tiger 를 낼 때 보기에 lion 을 반드시 넣습니다. 따로 배우면 각각은 알아도 둘을 나란히 두면 못 고르기 때문입니다. cat/hat, bear/pear, sheep/ship, mouse/mouth, three/tree 도 같은 이유로 짝을 지어 두었고, 이것만 모은 <b>헷갈리는 짝</b> 주제도 따로 있습니다.',
          '<b>틀려도 목숨이 줄지 않습니다.</b> 시간 제한도 없습니다. 학습 게임에서 실수를 벌하면 아이는 생각하기가 아니라 찍기와 회피를 배웁니다. 틀리면 무엇을 골랐는지 알려 주고(“그건 사자(lion)예요”), 정답을 <b>천천히</b> 한 번 더 들려준 뒤 다시 고르게 합니다. 한 번에 맞히면 별 2개, 다시 찾아도 1개 — 0점은 없습니다.',
          '주제는 동물·학교·가족·먹을 것·색깔·숫자·몸 일곱 가지에 헷갈리는 짝을 더해 여덟입니다. 맞힌 낱말은 아래쪽에 모여 동물원이 채워집니다. 이 사이트의 <a href="/learn/english-words/">영어 낱말 놀이터</a>와 같은 낱말·같은 발음을 쓰고, 익힌 표시도 함께 쌓입니다.',
        ],
        how: [
          '🔊 소리를 듣고, 맞는 그림을 누르세요',
          '🔊 를 누르면 몇 번이든 다시 들을 수 있습니다',
          '🐢 를 누르면 천천히 들려줍니다 — 따라 말할 때 쓰세요',
          '틀려도 괜찮습니다. 목숨도 시간 제한도 없습니다',
          '🏠 로 언제든 다른 주제를 고를 수 있습니다',
        ],
      },
    },
  },
...
// ⚠️★2026-08-27 수정: 기본 언어가 en → **ko** 로 바뀐 게 2026-08-05 인데 이 두 함수만
//   옛 규칙(`en` 이 접두사 없음)이 하드코딩된 채 남아 있었다. 두 가지가 동시에 깨져 있었다:
//     · ko → `/ko/games/...` = 지금은 **meta refresh 리다이렉트** 페이지다(느리고 크롤 낭비)
//     · en → `/games/...`   = **한국어 페이지**다. 영어 방문자가 한글 화면에 떨어졌다
//   `blogUrl`·`shopUrl` 이 쓰는 `localizeUrl` 과 같은 규칙으로 통일한다.
//   ⚠️`src/i18n/tools.ts` 의 `toolUrl`·`toolsIndexUrl` 에 **같은 버그가 그대로 남아 있다.**
export function gameUrl(slug: string, lang: Lang): string {
  return localizeUrl(`/games/${slug}/`, lang);
}
export function gamesIndexUrl(lang: Lang): string {
  return localizeUrl('/games/', lang);
}
