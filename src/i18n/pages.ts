...
function about(lang: Lang): PageCopy {
  ...
  const by: Record<Lang, PageCopy> = {
    ...
    ko: {
      title: '소개',
      description: '미뉴소프트(menewsoft.com)는 AI로 만듭니다 — 동네 가게 홈페이지 제작, AI 기술 글, 아이 맞춤 학습 게임, 무료 웹게임. 누가 어떤 원칙으로 만드는지 소개합니다.',
      html: `
        <h1>미뉴소프트 소개</h1>
        <p class="lede">만드는 일은 AI로 합니다. 다만 무엇을 만들지와 그것이 쓸 만한지는 사람이 정합니다.</p>
        <p>미뉴소프트는 강원도 원주의 1인 스튜디오입니다. AI를 연구 대상이 아니라 <strong>연장</strong>으로 씁니다. 그림도, 목소리도, 코드도 AI로 만들고, 그렇게 만든 것이 실제로 쓰이는지를 직접 확인합니다. 여기 있는 것은 전부 그렇게 나왔습니다.</p>
        <h2>하는 일 네 가지</h2>
        <ul>
          <li><strong>가게 홈페이지 제작.</strong> 동네 가게의 소개 페이지를 만들고, 만든 뒤에도 소식을 이어서 전합니다. 사장님이 직접 공지를 쓸 수 있는 작성기를 함께 드리기 때문에, 개발자를 부르지 않아도 영업시간과 새 메뉴를 바꿀 수 있습니다. → <a href="/shops/">등록 업체</a></li>
          <li><strong>AI 기술 소개.</strong> 직접 만들어 보며 확인한 것을 글로 남깁니다. 되는 것만 쓰지 않고 안 되는 것도 그대로 씁니다. → <a href="/blog/">글</a></li>
          <li><strong>아이 맞춤 학습 게임 개발.</strong> 아이 이름과 학년, 배우는 과목에 맞춰 그 아이 한 명을 위한 게임을 만듭니다. 문제는 매번 새로 만들어지고 음성 안내가 붙어, 글을 다 못 읽는 아이도 혼자 할 수 있습니다. → <a href="${p.contact}">제작 문의</a></li>
          <li><strong>웹게임 무료 제공.</strong> 만든 게임은 설치도 가입도 없이 브라우저에서 바로 열립니다. 광고를 보고 기다릴 필요도 없습니다. → <a href="/games/">놀이공간</a></li>
        </ul>
        <h2>가게 홈페이지에서 지키는 원칙</h2>
        <ul>
          <li><strong>후기를 지어내지 않습니다.</strong> 손님 후기는 가게가 받은 것만 씁니다.</li>
          <li><strong>검색 1등을 약속하지 않습니다.</strong> 검색 순위는 저희가 정하는 것이 아닙니다.</li>
          <li><strong>가게 명의로 대신 쓰지 않습니다.</strong> 저희가 정리한 소식에는 반드시 <a href="${p.contact}">출처</a>를 답니다. 가게가 하지 않은 말을 손님이 믿고 움직이면 손님도 가게도 실제로 피해를 봅니다.</li>
          <li><strong>가격·영업시간에는 확인 날짜를 함께 적습니다.</strong> 시간이 지나면 틀릴 수 있다는 것을 숨기지 않습니다.</li>
        </ul>
        <h2>누가 만드나요</h2>
        <p>강원도 원주에 기반을 둔 현직 소프트웨어 개발자 <strong>menew</strong>가 만듭니다. 사람이 직접 만들고, 만든 사람이 직접 응대합니다. 잘못된 내용이 있으면 <a href="${p.contact}">알려주세요</a> — 확인해서 고칩니다.</p>
        <h2>지금 다루는 범위</h2>
        <p>가게 제작은 강원도에서 시작해 경기도·서울로 넓혀 가고 있습니다. 등록된 가게는 <a href="/shops/">등록 업체</a>에서 지역과 업종으로 찾아볼 수 있습니다.</p>
        <h2>만들고 싶은 것이 있으시다면</h2>
        <p>가게 소개 페이지든, 아이를 위한 학습 게임이든, 그 밖에 AI로 만들 수 있을 것 같은 무엇이든 <a href="${p.contact}">메일로 알려주세요</a>. 가능한지부터 솔직하게 답해 드립니다.</p>
      `,
    },
    ja: {
      title: '概要',
...
function contact(lang: Lang): PageCopy {
  ...
  const by: Record<Lang, PageCopy> = {
    ...
    ko: {
      ...
      html: `
        <h1>문의</h1>
        <p class="lede">가게 홈페이지 제작 문의, 등록 내용 정정 요청 — 편하게 연락 주세요.</p>
        <p>미뉴소프트는 <strong>menew</strong>가 직접 운영합니다. 만든 사람이 직접 받고 직접 답합니다:</p>
        <p style="font-size:1.15rem;"><strong><a href="mailto:${EMAIL}">${EMAIL}</a></strong></p>
        <h2>가게 홈페이지를 원하신다면</h2>
        <p>가게 사진 10~15장이면 소개 페이지를 만듭니다. 사장님이 직접 공지를 올릴 수 있는 작성기와 짧은 소개 영상 1편이 한 세트입니다. 실제로 만들어 둔 사례는 <a href="/shops/">등록 업체</a>에서 보실 수 있습니다.</p>
        <p>메일에 <strong>가게 이름·지역·업종·연락처</strong>를 적어 보내주시면 제작 범위와 비용을 안내해 드립니다.</p>
        <h2>등록된 내용이 틀렸다면</h2>
