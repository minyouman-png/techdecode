...
var SAVE_KEY = 'uja_quest_' + SUBJECT + '_v1';
...
function buildLevel(stage) {
  ...
  for (var i = 0; i < probs.length; i++) {
    ...
    for (var k = 0; k < choices.length; k++) {
      // ⚠️낱말 상자는 넓다 — 6칸(240px)씩 벌리고 순찰 폭을 좁혀야 서로 겹치지 않는다.
      var cx0 = (c - 6 + k * 6) * TILE + TILE / 2;
      if (mode === 'brick') {
        z.items.push({
        ...
        z.items.push({
          ...
          vx: (k % 2 ? 22 : -22), x0: cx0 - WORD_W / 2 - 22, x1: cx0 - WORD_W / 2 + 22, z: z
        });
      }
    }
    lv.zones.push(z);
