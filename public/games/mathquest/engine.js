  ...
  for (var i = 0; i < QUIZ_PER_STAGE; i++) {
    ...
    for (var k = 0; k < choices.length; k++) {
      if (mode === 'brick') {
        ...
        // ⚠️몬스터가 곧 '보기'다 — 사이에 **내려설 자리**가 없으면 아이는 아무거나 밟게 된다.
        //   2칸(80px) 간격에 몸통 38px, 순찰 ±14px 이면 최악일 때 틈이 14px 로,
        //   30px 인 아이가 물리적으로 들어갈 수 없었다. 간격을 2.5칸으로 벌리고 순찰을 줄인다.
        var mx = (c - 3.75 + k * 2.5) * TILE;
        z.items.push({
          kind: 'mob', v: choices[k], dead: false, wrong: 0, squash: 0,
          x: mx, y: GY - 40, w: 38, h: 40,
          hi: 7,                       // 판정은 그림보다 좁다(스치기만 해선 안 걸린다)
